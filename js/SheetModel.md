...
### Table of Contents:
* [Imports](#imports)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;Imports from other GE3 modules
* [Types](#types)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;[Flow](http://flow.org) typechecker type annotations
* [CreateNewSheet](#createnewsheet)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;A Convenience function for popping up a sheet in a new tab
* [SheetModel](#sheetmodel)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;A data structure representing the entire Sheet page; a container for SheetElements
* [SheetElement](#sheetelement)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;An abstract superclass representing a single displayed element,
    such as a CycleGraph visualizer or a Morphism between two visualizers
* [RectangleElement](#rectangleelement)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;The simplest kind of SheetElement: a colored rectangle.
* [TextElement](#textelement)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;A SheetElement subclass for showing text.
* [VisualizerElement](#visualizerelement)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;An abstract SheetElement subclass for showing a visualizer for a group
* [MTElement](#mtelement)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;A SheetElement subclass for showing the multiplication table of a group
* [CGElement](#cgelement)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;A SheetElement subclass for showing the cycle graph of a group
* [CDElement](#cdelement)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;A SheetElement subclass for showing the Cayley diagram of a group
* [WrappedCDView](#wrappedcdview)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;An internal class that shares a single WebGL context among all CDElements
* [ConnectingElement](#connectingelement)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;A SheetElement subclass for connecting two other SheetElements
* [MorphismElement](#morphismelement)
    <br>&nbsp;&nbsp;&nbsp;&nbsp;A SheetElement representing a homomorphism from one group to another
...
// className values for SheetElementJSON
...
// Common JSON used to create and position elements programmatically
...
// JSON used to generate RectangleElement
export type RectangleElementJSON = {
    color: color
} & SheetElementJSON;

// JSON used to generate TextElement
...
// JSON used to generate VisualizerElements
export type CDElementJSON = {
    arrows?: Array<groupElement>,	     // from ZmnInfo
    arrowColors?: Array<css_color>;	     // from ZmnInfo
    strategies?: Array<StrategyParameters>,  // from ZmnInfo
};
export type CGElementJSON = {
};
export type MTElementJSON = {
    elements?: Array<groupElement>,  // from SolvableInfo
};
export type VisualizerElementJSON = {
    groupURL: string,
    highlights?: {background: Array<css_color>} & Obj,  // Obj has highlights besides background
} & CDElementJSON & CGElementJSON & MTElementJSON & SheetElementJSON;

// JSON used to generate ConnectingElement
...
// JSON used to generate MorphismElement
...
A global convenience function for popping up a sheet in a new tab and building it from a big bolus of JSON.
...
A data structure representing the entire Sheet page; a container for SheetElements
...
The simplest SheetElement subclass: a colored rectangle.
...
A SheetElement subclass for showing text
...
An abstract SheetElement subclass for showing a visualizer for a group

```js
 */
export class VisualizerElement/*:: <VizDispJSON: Obj, VizDispType: VizDisplay<VizDispJSON>> */ extends SheetElement {
/*::
    group: XMLGroup;
    groupURL: string;
 ...
 */
    ...
    setVisualizerGroup ( group /*: XMLGroup */ ) { };
    ...
    constructor ( model /*: SheetModel */, groupURL /*: string */ ) {
        ...
        if ( groupURL ) { // may be absent if they will fill it using fromJSON()
            Library.getGroupOrDownload( groupURL )
                   .then( ( group ) => {
                       that.setVisualizerGroup( that.group = group );
                       that.rerender();
                   } )
                   .catch( Log.err )
        }
    ...
    isReady () { return (this.vizdisplay.group != undefined) }
    ...
    toJSON () /*: VisualizerElementJSON */ {
        ...
        if ( this.vizdisplay.group != undefined ) {
    ...
    toString () /*: string */ {
        const groupname = (this.vizdisplay.group != undefined) ? this.vizdisplay.group.name : 'a group';
...
VisualizerElement subclass for showing the multiplication table of a group

```js
 */
export class MTElement extends VisualizerElement/*:: <MulttableJSON, MulttableView> */ {
    constructor ( model /*: SheetModel */, groupURL /*: string */ ) {
...
VisualizerElement subclass for showing the cycle graph of a group

```js
 */
export class CGElement extends VisualizerElement/*:: <CycleGraphJSON, CycleGraphView> */ {
    constructor ( model /*: SheetModel */, groupURL /*: string */ ) {
        super( model, groupURL  );
...
This is an internal class that shares a single [CayleyDiagramView](./CayleyDiagramView.js) instance
among all *CDElements*. This is important because each *CayleyDiagramView* instance uses a [WebGL
context](https://developer.mozilla.org/en-US/docs/Web/API/WebGLRenderingContext) internally, and at
this time most browsers will only display sixteen WebGL contexts at a time.  This means that the
naive one *CDElement*, one *CayleyDiagramView* approach can only display sixteen *CDElements* per
Sheet, and is thus unable to display, for example, the subgroup lattice of S<sub>4</sub>.

**Sharing:**<br>
*WrappedCDView* shares its singleton *CayleyDiagramView* `WrappedCDView.view`among the *CDElements*
by storing the *CDElement's* graphic state as a JSON object rather than an active WebGL context.
When a *CDElement's* displayed image needs to be updated, the`getView()`method
configures`WrappedCDView.view`from the *CDElement's* stored group and JSON data, the graphic state
is updated (for example, by a call to `this.fromJSON()` as a result of a message from an
editor), and a call to`getImage()`creates a new &lt;img&gt;. `WrappedCDView.view`is then
available to be used by other *CDElements* (suitably reconfigured), or it can be reused by the same
*CDElement* without change (see the following).

**Reusing already-configured view:**<br>
The *CDElement* for which`WrappedCDView.view`is currently configured is referenced by
its`wrapper`attribute. When accessing the
shared`WrappedCDView.view,`if`WrappedCDView.view.wrapper`refers to`this`*CDElement*,
the`WrappedCDView.view`needs no further configuration: it already holds this *CDElement's* graphic
state and is ready to use as is. This situation arises especially when the *CDElement* is being
edited and a stream of messages from the editor updates the same element repeatedly.

**Initial page creation:**<br>
The most complex Sheets currently generated by GE3 are the Cayley diagram subgroup lattices created
in [SubgroupInfo](../SubgroupInfo.js). These can consist of upwards of 50 *CDElements* and would
take more time to display than it takes to load the entire GE3 library. Fortunately, the element
layout is the same in all the visualizers: only the element highlights differ. It is thus possible
for *WrappedCDView* to initialize`WrappedCDView.view`once for the first image, and then use it
to create all the remaining images by changing only the highlighting.  In order to recognize this
particular special case, during initialization (from object construction until the first call
to`getImage()`), calls to`setDiagram()`and`fromJSON()`are not actually executed: they simply stash
their arguments in the`initialParameters`object as its`group, diagram,`and`generatingJSON`attributes.
During the first call to`getImage(),`the contents of`initialParameters`and the configuration
of`WrappedCDView.view`are examined to see if it will generate the same group layout (same group, no
specified strategy or diagram). If so,`WrappedCDView.view`is used as it stands, without
reconfiguration.  The highlights are set from`initialParameters.generatingJSON,`the current graphic
state is converted to JSON with`WrappedCDView.view.toJSON()`and stored in`this.json`, and an image
is created by`WrappedCDView.view.getImage().` (The image is also cached in`lastImage`-- see below.)
If`WrappedCDView.view`is not suitably configured already, the stashed attributes are simply used to
configure it as usual. In any case, in the first call to`getImage(), initialParameters`is set
to`undefined`to indicate that initialization is over.

**Image reuse during page creation:**<br>
During initialization`getImage()`can be called more than once to return the same image. During the
first call the image generated by`WrappedCDView.view`is stored in`lastImage,`which is returned
in subsequent calls if available. It is cleared by the first call to`resetImage()`and is not used
after initialization.

**Node position caching**<br>
The position of each element in the unit square is cached in the array`this.unitSquarePositions.`
This is cleared by the`resetImage()`method whenever the image changes (`setDiagram()`,`setSize()`,
and`fromJSON()`), and is filled when accessed in`this.unitSquarePosition().` For convenience
a`getUnitSquarePositions()`method that returns the entire array of element positions was added to
*CayleyDiagramView*.

```js
*/
class WrappedCDView /*:: implements VizDisplay<CayleyDiagramJSON & VisualizerElementJSON> */ {
/*::
    static view: CayleyDiagramView & {wrapper: WrappedCDView};
    json: CayleyDiagramJSON;
    ...
    lastImage: ?Image;
*/
    constructor (options /*: {width: number, height: number} & Obj */) {
        // Initialize the singleton WrappedCDView.view if needed
        if (WrappedCDView.view == undefined) {
            WrappedCDView.view =
                ((createLabelledCayleyDiagramView(options) /*: any */) /*: CayleyDiagramView & {wrapper: WrappedCDView} */);
        }
        this.size = {w: options.width, h: options.height};
        this.initialParameters = {};
    }
    getImage () /*: Image */ {
        // See Initial page creation discussion above
        const initialParameters = this.initialParameters;
        if (initialParameters != undefined && initialParameters.hasOwnProperty('group')) {
            ...
            this.json = WrappedCDView.view.toJSON();
            return ((this.lastImage /*: any */) /*: Image */);
        } else if (this.lastImage != undefined) {
            return this.lastImage;
        } else {
            return this.getView().getImage();
        }
    }
    getSize () /*: {w: number, h: number} */ {
        return this.size;
    ...
    // Configure WrappedCDView.view for this CDElement's group and graphic state
    getView () /*: CayleyDiagramView */ {
        if (WrappedCDView.view.wrapper != this) {
        ...
        WrappedCDView.view.setSize(this.size.w, this.size.h);
        return WrappedCDView.view;
    }
    // Called when the image changes and these values become obsolete
    resetImage () {
        this.lastImage = undefined;
        this.unitSquarePositions = undefined;
    }
    // Sets Cayley diagram group and diagram name (optional)
    setDiagram (group /*: XMLGroup */, diagram /*: ?string */) {
        if (this.initialParameters != undefined) {  // See Initial page creation discussion above
            this.initialParameters.group = group;
    ...
    unitSquarePosition (element /*: groupElement */) /*: {x: number, y: number} */ {
        if (this.unitSquarePositions == undefined) {
            this.unitSquarePositions = this.getView().unitSquarePositions();  // See Node position caching discussion above
        }
        return this.unitSquarePositions[element];
    }
    // Returns complete graphic state of CayleyDiagramView
    toJSON () /*: CayleyDiagramJSON & VisualizerElementJSON */ {
        return ((this.json /*: any */) /*: CayleyDiagramJSON & VisualizerElementJSON */);
    }
...
VisualizerElement subclass for showing the Cayley diagram of a group

```js
 */
export class CDElement extends VisualizerElement/*:: <CayleyDiagramJSON & VisualizerElementJSON, WrappedCDView> */ {
    setVisualizerGroup ( group /*: XMLGroup */ ) {
...
A SheetElement subclass for connecting two other SheetElements
...
A ConnectingElement representing a homomorphism from one group to another

```js
 */
export class MorphismElement extends ConnectingElement/*:: < VisualizerElement<any, any> > */ {
/*::
    name: string;
    showManyArrows: boolean;
    showDomAndCod: boolean;
 ...
 */
    ...
    constructor ( model /*: SheetModel */, from /*: VisualizerElement<any, any> */, to /*: VisualizerElement<any, any> */ ) {
        super( model, from, to );
