...
function emitStateChange () {
   if ( !canEmit ) return;
   var json = ((graphicContext.toJSON( multtable ) /*: any */)
               /*: MulttableJSON & {_color_choice?: string, _subgroup_choice?: string | string[] | number} */ );
...
function dragOver(_event /*: JQueryEventObject */) {
   ...
   if (Drag_state.isDragging && !event.shiftKey) {
      ...
      if (location === undefined) {
         ...
         const position = ((Drag_state.position /*: any */) /*: {x: number, y: number} */);
...
function drop(_event /*: JQueryEventObject */) {
   ...
   event.preventDefault();
   if (Drag_state.isDragging && !event.shiftKey) {
      const position = ((Drag_state.position /*: any */) /*: [number, number] */);
