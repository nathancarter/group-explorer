 ...
 */
   ...
   static draw() {
      ...
      const num_strategies = CD.Cayley_Diagram[0].strategies.length;
      if (num_strategies == 0) {
         $('#generation-table').html(
            '<tr style="height: 3em"><td></td><td style="width: 25%"></td><td style="width: 40%"></td><td></td></tr>');
      } else {
         CD.Cayley_Diagram[0].strategies.forEach( (strategy, inx) =>
   ...
   static showGeneratorMenu(click_location /*: eventLocation */, strategy_index /*: number */) {
      ...
      const eligibleGenerators = ( (strategy_index == 0) ?
                                   new BitSet(CD.Group[0].order, [0]) :
                                   CD.Cayley_Diagram[0].strategies[strategy_index-1].bitset.clone() )
            .complement().toArray();
   ...
   static showAxisMenu(click_location /*: eventLocation */, strategy_index /*: number */) {
      ...
      const curvable =
         (CD.Cayley_Diagram[0].strategies[strategy_index].bitset.popcount()
            /  ((strategy_index == 0) ? 1 : CD.Cayley_Diagram[0].strategies[strategy_index - 1].bitset.popcount()))
      > 2;
   ...
   static showOrderMenu(click_location /*: eventLocation */, strategy_index /*: number */) {
      ...
      const makeStrategyList = () => {
         ...
         const result = CD.Cayley_Diagram[0].strategies
   ...
   static makeOrganizeByMenu() {
      ...
      const result = CD.Group[0].subgroups
           .reduce( (list, subgroup, inx) => {
              if (subgroup.order != 1 && subgroup.order != CD.Group[0].order) {  // only append non-trivial subgroups
   ...
   static organizeBy(subgroup_index /*: number */) {
      ...
      const subgroup_generators = CD.Group[0].subgroups[subgroup_index].generators.toArray();
   ...
   static updateGenerator(strategy_index /*: number */, generator /*: number */) {
      const strategies = CD.Cayley_Diagram[0].getStrategies();
   ...
   static setMult(rightOrLeft /*: string */) {
      CD.Cayley_Diagram[0].right_multiplication = (rightOrLeft == 'right');
      CD.Graphic_Context[0].showGraphic(CD.Cayley_Diagram[0]);
   }
}
...
function refineStrategies(strategies /*: StrategyArray */) {
   const generators_used = new BitSet(CD.Group[0].order);
   let elements_generated = new BitSet(CD.Group[0].order, [0]);
   strategies = strategies.reduce( (nonRedundantStrategies, strategy) => {
      if (!elements_generated.isSet(strategy[0])) {
         ...
         elements_generated = CD.Group[0].closure(generators_used);
   ...
   if (elements_generated.popcount() != CD.Group[0].order) {
      // look for new element -- we know one exists
      const new_generator = ((elements_generated
                              .complement()
                              .toArray()
                              .find( (el) => CD.Group[0].closure(generators_used.clone().set(el)).popcount() == CD.Group[0].order ) /*: any */) /*: groupElement */);
...
function updateStrategies(new_strategies /*: StrategyArray */) {
   const strategies = refineStrategies(new_strategies);
   CD.Cayley_Diagram[0].setStrategies(strategies);
   CD.Cayley_Diagram[0].removeLines();
   DC.Arrow.getAllArrows().forEach( (arrow) => CD.Cayley_Diagram[0].addLines(arrow) );
