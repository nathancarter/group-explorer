...
Note the call to [MathML preload](../js/MathML.md#caching) that loads the MathML cache. This is often desirable since it avoids having to perform many small asynchronous activities in subsequent MathML calls, which together with their wait times can take considerably longer than a single consolidated action at the start.
```javascript
*/
 ...
 */
       function completeSetup() {
          ...
          $('#heading').html(MathML.sans('<mtext>Visualizer Example for&nbsp;</mtext>' + Group.name));
          MathJax.Hub.Queue(['Typeset', MathJax.Hub, 'header']); // normally covered by preload, unneeded
      ...
      <div id="bodyDouble">
         ...
         <div id="controls">
            <div id="options">
               ...
               <button id="view-button" onclick="VC.showPanel('#view-control')">View</button>
            </div>
            ...
            <div id="view-control" class="panel">
               ...
               <template id="subgroup-choice-none-template">
                  <li action="choose(${index})" style="margin-right: 3em">${MathML.sansText('none')}</li>
               </template>
               <template id="subgroup-choice-template">
                  <li action="choose(${index})" style="margin-right: 3em">
