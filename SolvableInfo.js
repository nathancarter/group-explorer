...
function formatGroup() {
   ...
   if (group.isAbelian) {
      ...
      try {
         ...
         decompositionDisplay.push(makeGroupRef(((IsomorphicGroups.map.get(1) /*: any */) /*: Array<BasicGroup> */)[0]));
   ...
   $( '.show-decomposition' ).on( 'click', function ( event ) {
      ...
      const type = ((event.target /*: any */) /*: HTMLElement */).getAttribute( 'data-type' );
      showSolvableDecompositionSheet( ((type /*: any */) /*: string */) );
   } );
...
function showSolvableDecompositionSheet ( type /*: string */ ) {
   ...
   D.map( ( entry, index ) => {
      ...
      if ( previous ) {
         ...
         sheetElementsAsJSON.push( {
            ...
            w : W, h : H,
            highlights : {
               background : elementOrder.map( ( elt, idx ) => idx ? notred : red )
            }
         } );
